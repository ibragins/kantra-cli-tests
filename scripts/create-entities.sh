...
# Standalone tackle2-hub (CI setup-hub) exposes the REST API at the root URL.
# Operator/ingress deployments expose the API under /hub (auto-detected after auth).
hub_api_url() {
    echo "${1%/}/${2#/}"
}
...
json_id() {
    echo "$1" | jq -r '.id // empty' 2>/dev/null || true
}
...
check_hub_ready() {
    ...
    for i in {1..30}; do
        local http_code url
        for url in $(hub_ready_urls "$BASE_URL" applications); do
...
PROFILES_JSON=$(curl "${CURL_OPTS[@]}" "$(hub_api_url "$BASE_URL" analysis/profiles)" 2>/dev/null || echo "[]")
...
APPS_JSON=$(curl "${CURL_OPTS[@]}" "$(hub_api_url "$BASE_URL" applications)" 2>/dev/null || echo "[]")
...
if [ -n "$APP_ID" ] && [ "$APP_ID" != "null" ]; then
    ...
    TAGS_JSON=$(curl "${CURL_OPTS[@]}" "$(hub_api_url "$BASE_URL" tags)" 2>/dev/null || echo "[]")
...
ARCHETYPES_JSON=$(curl "${CURL_OPTS[@]}" "$(hub_api_url "$BASE_URL" archetypes)" 2>/dev/null || echo "[]")
...
if [ -n "$ARCHETYPE_ID" ] && [ "$ARCHETYPE_ID" != "null" ]; then
    ...
    TARGET_PROFILE_ID=$(echo "$HTTP_BODY" | jq -r '.profiles[0].id // empty' 2>/dev/null || true)
    TARGET_PROFILE_NAME=$(echo "$HTTP_BODY" | jq -r '.profiles[0].name // empty' 2>/dev/null || true)
...
echo ""
echo "Applications:"
curl "${CURL_VERIFY_OPTS[@]}" "$(hub_api_url "$BASE_URL" applications)" | jq '.[] | {id, name, description}'

echo ""
echo "Analysis Profiles:"
curl "${CURL_VERIFY_OPTS[@]}" "$(hub_api_url "$BASE_URL" analysis/profiles)" | jq '.[] | {id, name, description, targets: .rules.targets}'

echo ""
echo "Archetypes:"
curl "${CURL_VERIFY_OPTS[@]}" "$(hub_api_url "$BASE_URL" archetypes)" | jq '.[] | {id, name, description, profiles: .profiles | map({id, name, analysisProfile})}'
