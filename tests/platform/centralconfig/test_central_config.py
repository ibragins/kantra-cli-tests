...
from utils.command import build_central_config_login_command, build_central_config_sync_command, get_cli_path, \
    get_project_path, build_analysis_command_ccm
...
def test_sync_hub_central_config_negative(central_config_data):
...
def test_list_profiles_central_config(central_config_data):
    ...
    assert "profiles found in" in output.stdout.lower(), f"Expected 'profiles found in' in output, got: {output.stderr}"
